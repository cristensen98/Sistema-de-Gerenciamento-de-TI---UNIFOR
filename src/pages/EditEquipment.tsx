...
import { Save, X, AlertCircle, Download, Trash2 } from 'lucide-react';
...
export default function EditEquipment() {
  ...
  const fetchEquipment = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const equipment = data.find((e: any) => e.id === id);
        if (equipment) {
          setFormData({
            name: equipment.name || '',
            ...
            responsibilityTerm: equipment.responsibilityTerm || ''
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Erro de conexão ao atualizar equipamento');
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Editar Equipamento</h1>
        <p className="text-slate-500 mt-1 font-medium italic">Atualize as informações do item no inventário</p>
      </div>
        ...
        <div className="p-10 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-2">
              ...
              {formData.responsibilityTerm && (
                <div className="mb-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                      <Download className="w-5 h-5 text-primary" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-900 dark:text-white">termo_responsabilidade.pdf</p>
                      <p className="text-[10px] text-slate-500 font-medium">Arquivo anexado</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {(user?.role === 'Administrador' || user?.role === 'Técnico') && (
                      <a 
                        href={formData.responsibilityTerm} 
                        download="termo_responsabilidade.pdf" 
                        className="px-4 py-2 bg-primary text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-primary/90 transition-colors"
                      >
                        Baixar
                      </a>
                    )}
                    {user?.role === 'Administrador' && (
                      <button
                        type="button"
                        onClick={() => setFormData({ ...formData, responsibilityTerm: '' })}
